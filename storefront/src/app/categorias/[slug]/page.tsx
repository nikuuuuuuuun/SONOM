...
async function getProductsByCategory(slug: string) {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/store/products?category_id[]=${slug}`, {
      next: { revalidate: 60 },
    })
    if (!res.ok) return []
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-12 sm:px-6 lg:px-8">
        <h1 className="mb-2 text-3xl font-bold text-sonom-text">{categoryName}</h1>
        <p className="mb-8 text-sonom-muted">
          {products.length} producto{products.length !== 1 ? "s" : ""} en esta categoria
        </p>
